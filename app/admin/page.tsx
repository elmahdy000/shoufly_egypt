    ...
    <div className="min-h-full bg-[#F1F5F9] pb-32 font-sans text-right" dir="rtl">
      ...
      <section className="bg-slate-950 text-white border-b-8 border-primary sticky top-0 z-40 overflow-hidden shadow-2xl">
        ...
        <div className="w-full px-8 lg:px-12 py-10 relative z-10">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
            <div className="space-y-4">
               <div className="flex items-center gap-3">
                  <div className="w-3 h-3 rounded-full bg-primary shadow-[0_0_15px_rgba(6,182,212,0.8)]" />
            ...
            <div className="flex gap-4 p-4 bg-white/5 border-2 border-white/10 rounded-[2rem] shadow-2xl">
               <div className="px-8 py-3 text-center border-l-2 border-white/10 last:border-l-0">
                  ...
                  <p className="text-lg font-black text-emerald-400 flex items-center gap-2 justify-center italic"><Zap size={16} /> ONLINE</p>
               </div>
      ...
      <div className="w-full px-8 lg:px-12 py-12 space-y-12">
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            ...
            <AnMetric label="الطلبات المفتوحة" val={stats?.openRequests ?? 0} icon={AlertCircle} color="bg-amber-600" delay={0.2} />
            <AnMetric label="طلبات الـ اليوم" val={stats?.todayRequests ?? 0} icon={Package} color="bg-slate-900" delay={0.3} />
                 ...
                 <table className="w-full text-right border-collapse">
                    <thead>
                       <tr className="bg-slate-50 text-slate-500 border-b border-slate-100 italic">
                          ...
                          <th className="px-8 py-6 text-[11px] font-black uppercase tracking-widest text-right">المبادر</th>
                          <th className="px-8 py-6 text-[11px] font-black uppercase tracking-widest text-left">التوقيت</th>
                       </tr>
                    </thead>
                    <tbody className="divide-y-4 divide-slate-50">
                       {loading ? (
                         ...
                         <tr><td colSpan={4} className="py-32 text-center font-black text-slate-300 text-3xl opacity-20 italic">لا توجد عمليات نشطة</td></tr>
                       ) : (
                         stats!.recentRequests.map(req => (
           ...
           <div className="lg:col-span-4 space-y-8">
              <div className="bg-slate-950 rounded-[2.5rem] p-10 text-white shadow-2xl border-r-8 border-primary relative overflow-hidden group">
                 ...
                 <div className="space-y-4 relative z-10">
                    ...
                    <AdmQuickLink href="/admin/tracking" icon={LayoutDashboard} label="مراقبة اللوجستيات" />
                    <AdmQuickLink href="/admin/users" icon={Users} label="إدارة المستخدمين" />
                 </div>
              </div>
              ...
              <div className="bg-white border-4 border-slate-950 rounded-[2.5rem] p-10 shadow-xl space-y-8">
                 ...
                 <div className="space-y-4">
                    <StatusLine label="طلبات اليوم" val={stats?.todayRequests ?? 0} />
                    <StatusLine label="إجمالي الأعضاء" val={stats?.totalUsers ?? 0} />
                    <StatusLine label="الموردين النشطين" val={stats?.totalVendors ?? 0} />
                 </div>
              </div>
              ...
              <div className="bg-emerald-500 rounded-[2.5rem] p-8 border-4 border-slate-950 flex items-center justify-between shadow-2xl hover:scale-[1.02] transition-all">
                 <div className="space-y-2">
                    <p className="text-[10px] font-black text-slate-950 uppercase tracking-[0.3em]">المبيعات</p>
